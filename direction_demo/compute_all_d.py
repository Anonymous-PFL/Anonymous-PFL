from cal_d import get_GPFL_d, get_d_mgdaplus_d, get_MGDA_d, get_FedFV_d
import torch
import copy
import numpy as np
...
alpha = 0
d1, descent_flag = get_FedFV_d(copy.deepcopy(grad_mat), l_locals, alpha, device)
d1 /= torch.norm(d1)
c = -(grad_mat @ d1.T)
...
d2, descent_flag = get_d_mgdaplus_d(copy.deepcopy(grad_mat/torch.norm(grad_mat, dim=1).reshape(-1, 1)), epsilon, lambda0, device)
d2 /= torch.norm(d2)
c = -(grad_mat @ d2.T)
if not torch.all(c <= 1e-5):
...
d, Q, g, fair_grad = get_GPFL_d(copy.deepcopy(grad_mat), l_locals, None, p, miu, device)
print('fair grad: ', fair_grad)
d = d / torch.norm(d)
c = -(grad_mat @ d.T)
print(c)
if not torch.all(c <= 1e-5):
    print('FedMGDP: not the descent direction')
    print(c)
print('d: ', d)
print()
