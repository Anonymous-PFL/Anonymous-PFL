...
class Client:
    ...
    def get_message(self, msg):
        ...
        if msg['command'] == 'require_client_per_model':
            ...
            if self.dishonest is not None:
                if self.dishonest['grad norm'] is not None:
                    return_model = (return_model - self.old_per_model) * self.dishonest['grad norm'] + self.old_per_model
                if self.dishonest['zero grad'] is not None:
        ...
        if msg['command'] == 'require_multi_model_weights':  
            ...
            for idx, model in enumerate(self.model_list):
                ...
                if self.dishonest is not None:
                    if self.dishonest['grad norm'] is not None:
                        return_model = (return_model - self.old_model_list[idx]) * self.dishonest['grad norm'] + self.old_model_list[idx]
                    if self.dishonest['zero grad'] is not None:
        ...
        if msg['command'] == 'require_test_result':
            ...
            return_msg['per_model_metric_history'] = copy.deepcopy(self.per_model_metric_history)
            return return_msg
    ...
    def train(self, epochs):
        self.optimizer = self.train_setting['optimizer'].__class__(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.lr)
        self.optimizer.defaults = copy.deepcopy(self.train_setting['optimizer'].defaults)
    ...
    def test(self):
        ...
        self.metric_history['training_loss'].append(float(self.model_loss) if self.model_loss is not None else None)
        ...
        with torch.no_grad():
            ...
            for (batch_x, batch_y) in self.local_test_data:
                ...
                if self.per_model is not None:
                    out = self.model_forward(self.per_model, batch_x)  
                    loss = criterion(out, batch_y).item()
                    per_model_metric_dict['test_loss'] += float(loss) * batch_y.shape[0]  
                    for metric in self.metric_list:
                        per_model_metric_dict[metric.name] += metric.calc(out, batch_y)
            ...
            if self.per_model is not None:
                self.per_model_metric_history['test_loss'].append(metric_dict['test_loss'] / self.local_test_number)
                for metric in self.metric_list:
                    self.per_model_metric_history[metric.name].append(per_model_metric_dict[metric.name])
                    if metric.name == 'correct':
                        self.per_model_metric_history['test_accuracy'].append(100 * per_model_metric_dict['correct'] / self.local_test_number)
            ...
            for (batch_x, batch_y) in self.global_test_data:
                ...
                out = self.model(batch_x)
                loss = criterion(out, batch_y).item()
                global_test_metric_dict['test_loss'] += float(loss) * batch_y.shape[0]  
                for metric in self.metric_list:
                    global_test_metric_dict[metric.name] += metric.calc(out, batch_y)
                
                if self.per_model is not None:
                    out = self.model_forward(self.per_model, batch_x)
                    loss = criterion(out, batch_y).item()
                    per_model_global_test_metric_dict['test_loss'] += float(loss) * batch_y.shape[0]  
                    for metric in self.metric_list:
                        per_model_global_test_metric_dict[metric.name] += metric.calc(out, batch_y)
            
            self.metric_history['global_test_loss'].append(global_test_metric_dict['test_loss'] / self.global_test_number)
            for metric in self.metric_list:
                self.metric_history['global_test_' + metric.name].append(global_test_metric_dict[metric.name])
                if metric.name == 'correct':
                    self.metric_history['global_test_accuracy'].append(100 * global_test_metric_dict['correct'] / self.global_test_number)
            if self.per_model is not None:
                self.per_model_metric_history['global_test_loss'].append(per_model_global_test_metric_dict['test_loss'] / self.global_test_number)
                for metric in self.metric_list:
                    self.per_model_metric_history['global_test_' + metric.name].append(per_model_global_test_metric_dict[metric.name])
                    if metric.name == 'correct':
                        self.per_model_metric_history['global_test_accuracy'].append(100 * per_model_global_test_metric_dict['correct'] / self.global_test_number)
            ...
            for (batch_x, batch_y) in self.simulated_test_data:
                ...
                out = self.model(batch_x)
                loss = criterion(out, batch_y).item()
                simulated_test_metric_dict['test_loss'] += float(loss) * batch_y.shape[0]  
                for metric in self.metric_list:
                    simulated_test_metric_dict[metric.name] += metric.calc(out, batch_y)
                
                if self.per_model is not None:
                    out = self.model_forward(self.per_model, batch_x)
                    loss = criterion(out, batch_y).item()
                    per_model_simulated_test_metric_dict['test_loss'] += float(loss) * batch_y.shape[0]  
                    for metric in self.metric_list:
                        per_model_simulated_test_metric_dict[metric.name] += metric.calc(out, batch_y)
            
            self.metric_history['simulated_test_loss'].append(simulated_test_metric_dict['test_loss'] / self.simulated_test_number)
            for metric in self.metric_list:
                self.metric_history['simulated_test_' + metric.name].append(simulated_test_metric_dict[metric.name])
                if metric.name == 'correct':
                    self.metric_history['simulated_test_accuracy'].append(100 * simulated_test_metric_dict['correct'] / self.simulated_test_number)
            if self.per_model is not None:
                self.per_model_metric_history['simulated_test_loss'].append(per_model_simulated_test_metric_dict['test_loss'] / self.simulated_test_number)
                for metric in self.metric_list:
                    self.per_model_metric_history['simulated_test_' + metric.name].append(per_model_simulated_test_metric_dict[metric.name])
                    if metric.name == 'correct':
                        self.per_model_metric_history['simulated_test_accuracy'].append(100 * per_model_simulated_test_metric_dict['correct'] / self.simulated_test_number)
