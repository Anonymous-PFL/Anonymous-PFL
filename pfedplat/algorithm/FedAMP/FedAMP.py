...
class FedAMP(fp.Algorithm):
    def __init__(self,
        ...
        if save_name is None:
            save_name = name + ' ' + model.name + ' E' + str(epochs) + ' lr' + str(train_setting['optimizer'].defaults['lr']) + ' decay' + str(train_setting['lr_decay']) + ' alphaK' + str(alphaK) + ' sigma' + str(sigma) + ' lam' + str(lam)
        ...
        if client_num is None and client_list is not None:
            ...
            client_list = [Client(i, copy.deepcopy(model), device, train_setting, metric_list, data_loader.input_data_shape, data_loader.target_class_num, alphaK, lam) for i in range(client_num)]  
