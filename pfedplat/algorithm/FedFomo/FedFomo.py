...
class FedFomo(fp.Algorithm):
    def __init__(self,
        ...
        if client_num is None and client_list is not None:
            ...
            client_list = [Client(i, copy.deepcopy(model), device, train_setting, metric_list, data_loader.input_data_shape, data_loader.target_class_num, client_num) for i in range(client_num)]  
    ...
    def send_models(self, m_locals, client_id_list, weight_vector_list):
        ...
        for idx, client in enumerate(self.online_client_list):
            ...
            choose_indices = torch.topk(self.P[client_id_list[idx]][client_id_list], self.online_client_num).indices.tolist()  
            model_list = []  
            id_list = []  
            for i in choose_indices:
                model_list.append(m_locals[i])
                id_list.append(client_id_list[i])
            msg = {'command': 'get_models', 'model_list': model_list, 'id_list': id_list}
...
class Client(fp.Client):
    ...
    def weight_cal(self):
        ...
        for client_model in self.client_model_list:
            ...
            weight_list.append(float(((loss - self.cal_all_batches_loss(client_model)) / (torch.norm(params_diff) + 1e-6)).item()))
