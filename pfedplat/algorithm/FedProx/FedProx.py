...
class FedProx(fp.Algorithm):
    def __init__(self,
        ...
        if save_name is None:
            save_name = name + ' ' + model.name + ' E' + str(epochs) + ' lr' + str(train_setting['optimizer'].defaults['lr']) + ' decay' + str(train_setting['lr_decay']) + ' mu' + str(mu)
        ...
        if client_num is None and client_list is not None:
            ...
            client_list = [Client(i, copy.deepcopy(model), device, train_setting, metric_list, mu) for i in range(client_num)]  
...
class Client(fp.Client):
    def __init__(self,
        ...
        self.optimizer = pgd(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.lr, mu=mu)
