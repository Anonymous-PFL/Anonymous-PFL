...
class FedROD(fp.Algorithm):
    def __init__(self,
        ...
        if client_num is None and client_list is not None:
            ...
            client_list = [Client(i, copy.deepcopy(model), device, train_setting, metric_list, data_loader.target_class_num) for i in range(client_num)]  
    ...
    def train(self, epochs):
        ...
        for e in range(epochs):
            for step, (batch_x, batch_y) in enumerate(self.local_training_data):
                ...
                loss = self.balanced_softmax_loss(batch_y, out, self.sample_per_class)
