...
class pFedMe(fp.Algorithm):
    def __init__(self,
        ...
        if save_name is None:
            save_name = name + ' ' + model.name + ' E' + str(epochs) + ' lr' + str(train_setting['optimizer'].defaults['lr']) + ' decay' + str(train_setting['lr_decay']) + ' beta' + str(beta) + ' lam' + str(lam)
    ...
    def aggregate(self, w_locals):
        ...
        for k in averaged_params.keys():
            ...
            w_global[k] = (1 - self.beta) * w_global[k] + self.beta * averaged_params[k]
    ...
    def step(self, old_model_params):
        for group in self.param_groups:
            for p, old_weight in zip(group['params'], old_model_params):
                ...
                p.data = p.data - group['lr'] * (p.grad.data + group['lam'] * (p.data - old_weight.data) + group['mu'] * p.data)
...
class Client(fp.Client):
    def __init__(self,
        ...
        self.optimizer = pFedMeOptimizer(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.lr, lam=lam)
        self.lam = lam
        ...
        for e in range(epochs):
            for step, (batch_x, batch_y) in enumerate(self.local_training_data):
                ...
                for theta_tilde_param, per_model_parameter in zip(self.model.parameters(), per_model_parameters):
                    per_model_parameter.data = per_model_parameter.data - self.lam * self.lr * (per_model_parameter.data - theta_tilde_param.data)
